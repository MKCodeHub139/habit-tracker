...
setThisMonthsHabit?.forEach((habit)=>{
  ...
  return (
    <div className='main bg-fuchsia-100 min-h-screen p-5 rounded-2xl'>
 <div className="head">
    ...
    {setThisMonthsHabit?.map((habit)=>{
      return(
        <div className="habit-card w-full bg-fuchsia-200 p-3 rounded-2xl">
          ...
          <div className="habit-progress w-full  flex items-center  mt-2">
            <input type="range" name="" id=""  className='w-[100%] appearance-none bg-black h-2 rounded-full [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:h-5 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-blue-600 [&::-webkit-slider-thumb]:border-2 'min={0} max={thisMonthLastDate} value={habit?.thisMonthCompleted?.length} />
          </div>
        </div>
      )
    })}
